...
public class ExportController {
    ...
    public ResponseEntity<byte[]> exportTickets(
        ...
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=tickets_" + LocalDate.now() + ".xlsx")
                .contentType(
                        MediaType.parseMediaType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"))
                .body(outputStream.toByteArray());
    }
    ...
    public ResponseEntity<byte[]> exportFinancialReport(
        ...
        ByteArrayOutputStream outputStream = excelExportService.exportFinancialReport(societyId, reportType, startDate,
                endDate);
