...
export default function HeroSection({
  ...
  return (
    <>
      ...
      `}</style>
      <section
        className="relative mb-6 overflow-hidden rounded-2xl p-[3px] shadow-sm transition-all duration-300"
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            animation: "border-flow var(--flow-border-speed) linear infinite",
        ...
        <div className="relative flex h-full w-full flex-col overflow-hidden rounded-[13px] bg-[var(--bg-secondary)]">
          <div className="relative flex flex-col gap-0 md:flex-row">
            ...
            <aside
              className="flex shrink-0 flex-col items-center justify-center gap-6 border-t border-[var(--border-default)]/60 p-6 md:border-l md:border-t-0 lg:gap-8 lg:p-8 xl:flex-row"
              aria-label="Weather and time"
            >
              {/* Weather block */}
              <div className="flex flex-col items-center gap-3 lg:gap-4 xl:items-start">
                {locationName && (
                  <div className="flex items-center gap-2 text-[var(--text-secondary)]">
                    <MapPin className="h-4 w-4 shrink-0 text-[var(--accent-primary)] lg:h-5 lg:w-5" aria-hidden="true" />
                    <span className="truncate text-sm font-semibold lg:text-base">{locationName}</span>
                  </div>
                )}
                <div className="flex items-center gap-4">
                  <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl border border-[var(--border-default)] bg-[var(--bg-tertiary)] shadow-sm shadow-amber-500/10 lg:h-16 lg:w-16">
                    {weather?.current?.weather_code !== undefined ? (
                      <div className="[&>svg]:h-8 [&>svg]:w-8 lg:[&>svg]:h-10 lg:[&>svg]:w-10">{getWeatherIcon(weather.current.weather_code)}</div>
                    ) : (
                      <Sun className="h-8 w-8 text-amber-500 lg:h-10 lg:w-10" aria-hidden="true" />
                    )}
                  </div>
                  <div className="flex flex-col items-start pt-1">
                    <p className="tabular-nums text-[36px] font-black leading-none tracking-tight text-amber-600 dark:text-amber-400 lg:text-[46px]">
                      {weather?.current?.temperature_2m != null ? `${Math.round(weather.current.temperature_2m)}°C` : "-"}
                    </p>
                    <p className="mt-1 text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] lg:mt-1.5 lg:text-sm">
                      {weather?.current?.weather_code !== undefined ? getWeatherDesc(weather.current.weather_code) : "Fetching..."}
                    </p>
                  </div>
                </div>
              </div>

              <div className="hidden h-[80px] w-px bg-[var(--border-default)]/60 xl:block" />
              <div className="block h-px w-full bg-[var(--border-default)]/60 xl:hidden" />
