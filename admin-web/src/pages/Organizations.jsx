...
import {
  ...
  ChevronRight, MapPin, Users, Eye, EyeOff, Shield, TrendingUp,
  Mail, Phone, User
} from 'lucide-react'
...
export default function Organizations() {
  ...
  const maxSocietiesRef = useRef(null)
  ...
  // Fetch societies for expanded org
  const { data: orgSocieties = [] } = useQuery({
    queryKey: ['org-societies', expandedOrg],
    queryFn: () => societyApi.getByOrganizationId(expandedOrg).then(res => res.data).catch(() => []),
    enabled: !!expandedOrg,
  })
  ...
  const handleSubmit = (e) => {
    ...
    const data = {
      ...
      ...(!editingOrg && formData.get('ownerPassword')
      ...
      {isLoading ? (
        ...
        <div className="org-grid">
          {filteredOrganizations.map((org) => {
            ...
            return (
              <div key={org.id} className={`org-card ${isExpanded ? 'org-card--expanded' : ''}`}>
                ...
                {/* Expand to show societies */}
                <button className="org-card__expand-btn" onClick={() => toggleExpand(org.id)}>
                  <span>{isExpanded ? 'Hide' : 'View'} Societies</span>
                  <ChevronRight size={16} className={`org-card__chevron ${isExpanded ? 'org-card__chevron--open' : ''}`} />
                </button>

                {isExpanded && (
                  <div className="org-card__societies">
                    {orgSocieties.length === 0 ? (
                      <p className="org-card__no-societies">No societies yet</p>
                    ) : (
                      orgSocieties.map(s => (
                        <div
                          key={s.id}
                        ...
                        >
                          <div className="org-card__society-info">
                            <Building2 size={14} />
                            <span className="org-card__society-name">{s.name}</span>
                          </div>
                          <div className="org-card__society-meta">
                            {s.city && <span><MapPin size={11} /> {s.city}</span>}
                            <span><Users size={11} /> {s.actualFlats || 0} units</span>
                          </div>
